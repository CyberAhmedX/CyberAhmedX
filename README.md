<h1 align="left">Hi, I'm Ahmed Sajid Butt 👋</h1>

**SOC Analyst L1 / Detection Engineer** based in Lahore, Pakistan. I investigate incidents, hunt for what alerts miss, and build detection content from the ground up.

- 🔭 Currently building detection labs across **Wazuh, Microsoft Defender XDR, and the ELK Stack**
- 🧪 Recent work: [**Vigil**](https://github.com/CyberAhmedX/Vigil), a SOC detection lab simulating a four-stage web app compromise with a working Wazuh-to-TheHive escalation workflow
- 🎓 Completed a hands-on cybersecurity internship investigating real incidents, threat hunting with KQL, and onboarding endpoints to Microsoft Defender for Endpoint
- 🌐 Full portfolio: **[cyberahmedx.github.io](https://cyberahmedx.github.io)**
- 📫 Reach me: [LinkedIn](https://linkedin.com/in/a-sajid) · [Email](mailto:ahmedsajid_bw@proton.me)

---

### 🛠️ Tools & Technologies

![Microsoft Defender](https://img.shields.io/badge/Microsoft%20Defender%20XDR-0078D4?style=flat-square&logo=microsoft&logoColor=white)
![Wazuh](https://img.shields.io/badge/Wazuh-3AB0FF?style=flat-square&logo=wazuh&logoColor=white)
![Elastic](https://img.shields.io/badge/ELK%20Stack-005571?style=flat-square&logo=elastic&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Windows Server](https://img.shields.io/badge/Windows%20Server-0078D6?style=flat-square&logo=windows&logoColor=white)
![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=flat-square&logo=powershell&logoColor=white)
![KQL](https://img.shields.io/badge/KQL-00A4EF?style=flat-square&logo=microsoftazure&logoColor=white)

---

### 📌 Featured Work

| Project | Description |
|---|---|
| 🔎 [**Vigil**](https://github.com/CyberAhmedX/Vigil) | SOC detection lab: Wazuh, TheHive, OpenSearch, Docker, four-stage attack simulation mapped to MITRE ATT&CK |
| 🛡️ [**Sentinel-Forge**](https://github.com/CyberAhmedX/Sentinel-Forge) | ELK-based SOC home lab with custom Kibana detection rules and Mythic C2 adversary simulation |

---

### 📊 GitHub Stats

![Ahmed's GitHub stats](https://github-readme-stats.vercel.app/api?username=CyberAhmedX&show_icons=true&theme=github_dark&hide_border=true)

